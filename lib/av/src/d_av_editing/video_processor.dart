...
abstract class VideoProcessor {
  ...
  static Future<VideoEditorController?> initializeVideoEditorController({
    ...
    if (file != null){
      ...
      _controller = VideoEditorController.file(
        ...
        coverStyle: VideoProcessor.getCoverStyle,
        coverThumbnailsQuality: 100,
        cropStyle: VideoProcessor.getCropStyle,
        trimStyle: VideoProcessor.getTrimStyle,
  ...
  static Future<File?> exportVideo({
    ...
    if (videoEditorController != null){
      ...
      int? _bitRate = await VideoProcessor.readFileBitrate(file: videoEditorController.file);
      _bitRate ??= 1000000;
  ...
  static bool checkCurrentTimeIsOutOfTrimRange({
    ...
    /// START
    final int _start = VideoProcessor.getTrimTimeMinMs(controller: controller);
    final bool _isBeforeStart = _currentMs <= _start;

    /// END
    final int _end = VideoProcessor.getTrimTimeMaxMs(controller: controller);
  ...
  static Future<void> reportFileChange({
    ...
    final int? _bitRateBefore = await VideoProcessor.readFileBitrate(file: fileBefore);
    final double? _kiloWas = FileSizer.getFileSizeWithUnit(file: fileBefore, unit: FileSizeUnit.kiloByte);

    final int? _bitRateAfter = await VideoProcessor.readFileBitrate(file: fileAfter);
