...
abstract class _Calculator {
  ...
  Rect moveRect(Rect original, double deltaX, double deltaY, Rect imageRect) {
    ...
    double _deltaX = deltaX;
    double _deltaY = deltaY;

    if (original.left + _deltaX < imageRect.left) {
      _deltaX = (original.left - imageRect.left) * -1;
    }

    if (original.right + _deltaX > imageRect.right) {
      _deltaX = imageRect.right - original.right;
    }

    if (original.top + _deltaY < imageRect.top) {
      _deltaY = (original.top - imageRect.top) * -1;
    }

    if (original.bottom + _deltaY > imageRect.bottom) {
      _deltaY = imageRect.bottom - original.bottom;
    }
  ...
  Rect moveTopLeft(Rect original, double deltaX, double deltaY, Rect imageRect, double? aspectRatio) {

    final newLeft = max(imageRect.left, min(original.left + deltaX, original.right - 40));

    final newTop = min(max(original.top + deltaY, imageRect.top), original.bottom - 40);
  ...
  Rect moveTopRight(Rect original, double deltaX, double deltaY, Rect imageRect, double? aspectRatio) {

    final newTop = min(max(original.top + deltaY, imageRect.top), original.bottom - 40);

    final newRight = max(min(original.right + deltaX, imageRect.right), original.left + 40);
  ...
  Rect moveBottomLeft(Rect original, double deltaX, double deltaY, Rect imageRect, double? aspectRatio) {

    final newLeft = max(imageRect.left, min(original.left + deltaX, original.right - 40));

    final newBottom = max(min(original.bottom + deltaY, imageRect.bottom), original.top + 40);
  ...
  Rect moveBottomRight(Rect original, double deltaX, double deltaY, Rect imageRect, double? aspectRatio) {

    final newRight = min(imageRect.right, max(original.right + deltaX, original.left + 40));
